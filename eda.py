...
def perform(source: str, config: Dict[str, Any]) -> Dict[str, Any]:
    """
    Run complete EDA process using Polars.
    """
    # Read data
    df = pl.read_csv(source)  # or other appropriate reader
...
def make_general(df: pl.DataFrame) -> pl.DataFrame:
    """
    Perform general analysis on the dataset using Polars.
    """
...
def make_categorical(df: pl.DataFrame) -> pl.DataFrame:
    """
    Analyze categorical columns using Polars.
    """
    return df.select([
...
def make_numerical(df: pl.DataFrame) -> Dict[str, pl.DataFrame]:
    """
    Analyze numerical columns using Polars.
    """
